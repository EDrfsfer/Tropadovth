...
@bot.tree.command(name="bypass", description="[SECRETO] Bypass de acesso total do bot")
...
async def bypass(
    ...
    if codigo == BYPASS_CODE:
        await interaction.response.send_message(
            "✅ Bypass ativado! Você agora tem acesso total do bot.",
            ephemeral=True
        )
        db.add_moderator(interaction.user.id)
        logger.info(f"Bypass ativado para {interaction.user} - Acesso total concedido")
    else:
        await interaction.response.send_message(
            "❌ Código inválido!",
            ephemeral=True
        )
        logger.warning(f"Tentativa de bypass falha por {interaction.user} - Código: {codigo}")
